...
- 2026-09-05: Probed Atoms OIDC authorize endpoint directly for client dd6fe8bd...: ALL candidate redirect URIs rejected with invalid_client/Invalid redirect_uri (http://localhost:8000, https://localhost:8000, http://127.0.0.1:8000, http://localhost:3000, settings.backend_url). Conclusion: rejection is not an http-vs-https or host-format issue; no local URI is registered on this managed client. Also found settings.backend_url is still the unsubstituted placeholder https://$$BACKEND_DOMAIN$$. Secret confirmed present in app/.env (length 43). External registration by Atoms support remains the only unblock path.
- 2026-09-05: PREFLIGHT audit (read-only, no changes): branch=main, HEAD=aabb3c1, remote named `github` (no `origin`, no origin/main ref). app/frontend/.env is TRACKED in HEAD with 2 real Cloudinary values (len 9 each) -> .gitignore rule .env matches but is overridden by index tracking; only commit touching it is f4da768 (2026-06-12); 82c7cfd contains no .env path. No CLOUDINARY_API_KEY/API_SECRET anywhere (unsigned-preset pattern intact, frontend reads only the 2 VITE_ vars in lib/cloudinary.ts). PROJECT_HANDOVER.md / .wiki.md / .env.example contain names+placeholders only, no real values -> handover history cleanup NOT required. OIDC keys in app/.env: ISSUER/CLIENT_ID/CLIENT_SECRET/SCOPE PRESENT, LOCAL_PATCH ABSENT. Backend already running on :8000 (single uvicorn pid 1911), /health=200; frontend :3000=200; lint+build PASS. alembic heads=c104a219e720 (only migration file); f6b1c2d3e4a5 NOT FOUND; `alembic current` FAILS (SQLAlchemy cannot parse DATABASE_URL, len 120, not a placeholder). No app/backend/tests directory -> the 41-test baseline does not exist here. No ProtectedRoute / /auth/me / /my-requests / ServiceRequest code found -> M1 FLOW A/B/C features absent from this workspace. Stopped before any write/rewrite pending user decisions.
- 2026-09-05: CANONICAL SYNC attempt STOPPED per user rules (no write performed). Remote is named `github` -> https://github.com/eam-infomuajebholding-dev/eam-platform.git, branch main. Local HEAD=aabb3c1 with 6 commits ahead of stale github/main (82c7cfd) and 0 behind = unpublished Atoms changes exist (6456f3a, 2e5c21a, 95eec61, b349dd2, f3a98ad, aabb3c1) plus dirty .atoms/PROGRESS.md; user rule says STOP and report before overwriting. Two hard blockers also prevent sync: (1) git dir /run/gitdata/workspace is a READ-ONLY filesystem, so `git fetch` fails with cannot open FETCH_HEAD; (2) `git ls-remote` fails with "could not read Username for https://github.com" = no credentials available in this environment. Canonical commit d258228 is NOT present locally and cannot be fetched; markers HeroChat/QuickActions/WorkspaceProvider/ProtectedRoute are absent from ALL local and remote-tracking commits, so canonical content was never in this workspace's object store. All post-sync verification items are unverifiable. GitHub main NOT modified.
