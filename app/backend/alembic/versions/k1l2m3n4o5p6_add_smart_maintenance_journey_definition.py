"""Add Smart Maintenance (#13) journey definition.
...
from services.jos_seed import SMART_MAINTENANCE_WORKFLOW
...
def downgrade() -> None:
    op.execute(
        sa.text(
            "UPDATE journey_definitions SET is_active = 0 WHERE journey_type = 'smart_maintenance'"
        )
    )
