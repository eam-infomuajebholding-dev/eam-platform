import json
import logging
from typing import List, Optional

from datetime import datetime, date

from fastapi import APIRouter, Body, Depends, HTTPException, Query
from pydantic import BaseModel
from sqlalchemy.ext.asyncio import AsyncSession

from core.database import get_db
from dependencies.cms_write import require_cms_admin_write
from schemas.auth import UserResponse
from services.services import ServicesService

# Set up logging
logger = logging.getLogger(__name__)

router = APIRouter(prefix="/api/v1/entities/services", tags=["services"])


# ---------- Pydantic Schemas ----------
class ServicesData(BaseModel):
    """Entity data schema (for create/update)"""
    title: str
    description: str
    icon: str = None
    image_url: str = None
    page_path: str = None
    sort_order: int = None
    is_active: bool = None


class ServicesUpdateData(BaseModel):
    """Update entity data (partial updates allowed)"""
    title: Optional[str] = None
    description: Optional[str] = None
    icon: Optional[str] = None
    image_url: Optional[str] = None
    page_path: Optional[str] = None
    sort_order: Optional[int] = None
    is_active: Optional[bool] = None


class ServicesResponse(BaseModel):
    """Entity response schema"""
    id: int
    title: str
    description: str
    icon: Optional[str] = None
    image_url: Optional[str] = None
    page_path: Optional[str] = None
    sort_order: Optional[int] = None
    is_active: Optional[bool] = None
    created_at: Optional[datetime] = None
    updated_at: Optional[datetime] = None

    class Config:
        from_attributes = True


class ServicesListResponse(BaseModel):
    """List response schema"""
    items: List[ServicesResponse]
    total: int
    skip: int
    limit: int


class ServicesBatchCreateRequest(BaseModel):
    """Batch create request"""
    items: List[ServicesData]


class ServicesBatchUpdateItem(BaseModel):
    """Batch update item"""
    id: int
    updates: ServicesUpdateData


class ServicesBatchUpdateRequest(BaseModel):
    """Batch update request"""
    items: List[ServicesBatchUpdateItem]


class ServicesBatchDeleteRequest(BaseModel):
    """Batch delete request"""
    ids: List[int]


# ---------- Routes ----------
@router.get("", response_model=ServicesListResponse)
async def query_servicess(
    query: str = Query(None, description="Query conditions (JSON string)"),
    sort: str = Query(None, description="Sort field (prefix with '-' for descending)"),
    skip: int = Query(0, ge=0, description="Number of records to skip"),
    limit: int = Query(20, ge=1, le=2000, description="Max number of records to return"),
    fields: str = Query(None, description="Comma-separated list of fields to return"),
    db: AsyncSession = Depends(get_db),
):
    """Query servicess with filtering, sorting, and pagination"""
    logger.debug(f"Querying servicess: query={query}, sort={sort}, skip={skip}, limit={limit}, fields={fields}")
    
    service = ServicesService(db)
    try:
        # Parse query JSON if provided
        query_dict = None
        if query:
            try:
                query_dict = json.loads(query)
            except json.JSONDecodeError:
                raise HTTPException(status_code=400, detail="Invalid query JSON format")
        
        result = await service.get_list(
            skip=skip, 
            limit=limit,
            query_dict=query_dict,
            sort=sort,
        )
        logger.debug(f"Found {result['total']} servicess")
        return result
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error querying servicess: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")


@router.get("/all", response_model=ServicesListResponse)
async def query_servicess_all(
    query: str = Query(None, description="Query conditions (JSON string)"),
    sort: str = Query(None, description="Sort field (prefix with '-' for descending)"),
    skip: int = Query(0, ge=0, description="Number of records to skip"),
    limit: int = Query(20, ge=1, le=2000, description="Max number of records to return"),
    fields: str = Query(None, description="Comma-separated list of fields to return"),
    db: AsyncSession = Depends(get_db),
):
    # Query servicess with filtering, sorting, and pagination without user limitation
    logger.debug(f"Querying servicess: query={query}, sort={sort}, skip={skip}, limit={limit}, fields={fields}")

    service = ServicesService(db)
    try:
        # Parse query JSON if provided
        query_dict = None
        if query:
            try:
                query_dict = json.loads(query)
            except json.JSONDecodeError:
                raise HTTPException(status_code=400, detail="Invalid query JSON format")

        result = await service.get_list(
            skip=skip,
            limit=limit,
            query_dict=query_dict,
            sort=sort
        )
        logger.debug(f"Found {result['total']} servicess")
        return result
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error querying servicess: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")


@router.get("/{id}", response_model=ServicesResponse)
async def get_services(
    id: int,
    fields: str = Query(None, description="Comma-separated list of fields to return"),
    db: AsyncSession = Depends(get_db),
):
    """Get a single services by ID"""
    logger.debug(f"Fetching services with id: {id}, fields={fields}")
    
    service = ServicesService(db)
    try:
        result = await service.get_by_id(id)
        if not result:
            logger.warning(f"Services with id {id} not found")
            raise HTTPException(status_code=404, detail="Services not found")
        
        return result
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error fetching services {id}: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")


@router.post("", response_model=ServicesResponse, status_code=201)
async def create_services(
    data: ServicesData,
    db: AsyncSession = Depends(get_db),
    _admin: UserResponse = Depends(require_cms_admin_write),
):
    """Create a new services"""
    logger.debug(f"Creating new services with data: {data}")
    
    service = ServicesService(db)
    try:
        result = await service.create(data.model_dump())
        if not result:
            raise HTTPException(status_code=400, detail="Failed to create services")
        
        logger.info(f"Services created successfully with id: {result.id}")
        return result
    except ValueError as e:
        logger.error(f"Validation error creating services: {str(e)}")
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        logger.error(f"Error creating services: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")


@router.post("/batch", response_model=List[ServicesResponse], status_code=201)
async def create_servicess_batch(
    request: ServicesBatchCreateRequest,
    db: AsyncSession = Depends(get_db),
    _admin: UserResponse = Depends(require_cms_admin_write),
):
    """Create multiple servicess in a single request"""
    logger.debug(f"Batch creating {len(request.items)} servicess")
    
    service = ServicesService(db)
    results = []
    
    try:
        for item_data in request.items:
            result = await service.create(item_data.model_dump())
            if result:
                results.append(result)
        
        logger.info(f"Batch created {len(results)} servicess successfully")
        return results
    except Exception as e:
        await db.rollback()
        logger.error(f"Error in batch create: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Batch create failed: {str(e)}")


@router.put("/batch", response_model=List[ServicesResponse])
async def update_servicess_batch(
    request: ServicesBatchUpdateRequest,
    db: AsyncSession = Depends(get_db),
    _admin: UserResponse = Depends(require_cms_admin_write),
):
    """Update multiple servicess in a single request"""
    logger.debug(f"Batch updating {len(request.items)} servicess")
    
    service = ServicesService(db)
    results = []
    
    try:
        for item in request.items:
            # Only include non-None values for partial updates
            update_dict = {k: v for k, v in item.updates.model_dump().items() if v is not None}
            result = await service.update(item.id, update_dict)
            if result:
                results.append(result)
        
        logger.info(f"Batch updated {len(results)} servicess successfully")
        return results
    except Exception as e:
        await db.rollback()
        logger.error(f"Error in batch update: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Batch update failed: {str(e)}")


@router.put("/{id}", response_model=ServicesResponse)
async def update_services(
    id: int,
    data: ServicesUpdateData,
    db: AsyncSession = Depends(get_db),
    _admin: UserResponse = Depends(require_cms_admin_write),
):
    """Update an existing services"""
    logger.debug(f"Updating services {id} with data: {data}")

    service = ServicesService(db)
    try:
        # Only include non-None values for partial updates
        update_dict = {k: v for k, v in data.model_dump().items() if v is not None}
        result = await service.update(id, update_dict)
        if not result:
            logger.warning(f"Services with id {id} not found for update")
            raise HTTPException(status_code=404, detail="Services not found")
        
        logger.info(f"Services {id} updated successfully")
        return result
    except HTTPException:
        raise
    except ValueError as e:
        logger.error(f"Validation error updating services {id}: {str(e)}")
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        logger.error(f"Error updating services {id}: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")


@router.delete("/batch")
async def delete_servicess_batch(
    request: ServicesBatchDeleteRequest,
    db: AsyncSession = Depends(get_db),
    _admin: UserResponse = Depends(require_cms_admin_write),
):
    """Delete multiple servicess by their IDs"""
    logger.debug(f"Batch deleting {len(request.ids)} servicess")
    
    service = ServicesService(db)
    deleted_count = 0
    
    try:
        for item_id in request.ids:
            success = await service.delete(item_id)
            if success:
                deleted_count += 1
        
        logger.info(f"Batch deleted {deleted_count} servicess successfully")
        return {"message": f"Successfully deleted {deleted_count} servicess", "deleted_count": deleted_count}
    except Exception as e:
        await db.rollback()
        logger.error(f"Error in batch delete: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Batch delete failed: {str(e)}")


@router.delete("/{id}")
async def delete_services(
    id: int,
    db: AsyncSession = Depends(get_db),
    _admin: UserResponse = Depends(require_cms_admin_write),
):
    """Delete a single services by ID"""
    logger.debug(f"Deleting services with id: {id}")
    
    service = ServicesService(db)
    try:
        success = await service.delete(id)
        if not success:
            logger.warning(f"Services with id {id} not found for deletion")
            raise HTTPException(status_code=404, detail="Services not found")
        
        logger.info(f"Services {id} deleted successfully")
        return {"message": "Services deleted successfully", "id": id}
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error deleting services {id}: {str(e)}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Internal server error: {str(e)}")